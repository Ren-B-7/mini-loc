...
static int cb_complexity(int argc, char** argv, void* user_data)
{
    (void)argc;
    (void)argv;
    ((LocConfig*)user_data)->complexity_check = true;
    return 0;
}
...
__attribute__((cold)) void parse_cli(LocConfig* cfg, int argc, char** argv)
{
    ...
    cli_add_argument(&parser,
     (CliArgument){"--complexity", NULL, "Perform complexity checks",
         cb_complexity, cfg});
    cli_add_argument(&parser,
     (CliArgument){
    ...
    cli_add_argument(&parser,
     (CliArgument){"--output", "-o", "Output format: terminal, json, html, sql",
         cb_output, cfg});
    cli_add_argument(&parser,
     (CliArgument){
    ...
    cli_add_argument(&parser,
     (CliArgument){"--append", "-a", "Append custom language definitions",
         cb_append, cfg});
    cli_add_argument(&parser,
     (CliArgument){"--list-unknown", NULL, "List files with unknown extensions",
         cb_list_unknown, cfg});
    cli_add_argument(&parser,
